...
Cypress.Commands.add('login', (
  ...
  cy.session([email, password], () => {
    cy.request({
      method: 'POST',
      url: '/api/method/login', // cy.intercept('POST', '**/api/method/login').as('loginRequest');
      body: {
        usr: email,
        pwd: password,
      },
...
// /**
//  * Finds a specific payment, clicks the reject button, and confirms the rejection.
//  */
// Cypress.Commands.add('rejectPayment', (poNumber, comment) => {
//   cy.log(`--- Rejecting Payment for PO: ${poNumber} ---`);
//   cy.findPaymentRow(poNumber).within(() => {
//     cy.get('[data-cy="payments-reject-button"]').scrollIntoView().click();
//   });
//   // Assuming a dialog appears for rejection comments
//   cy.get('body').find('textarea').clear().type(comment); // A generic selector, update if a data-cy is available
//   cy.get('body').find('button:contains("Confirm")').click();
// });

/**
 * Dynamically finds the first PO in the list, rejects it, and verifies the success message.
 * This command is self-contained and does not need a PO number as input.
 */
...
// /**
//  * Finds a specific payment, clicks the approve button, and confirms the approval.
//  */
// Cypress.Commands.add('approvePayment', (poNumber) => {
//   cy.log(`--- Approving Payment for PO: ${poNumber} ---`);
//   cy.findPaymentRow(poNumber).within(() => {
//     cy.get('[data-cy="payments-approve-button"]').scrollIntoView().click();
//   });
...
/**
 * Dynamically finds the first available PO, approves it, and critically,
 * extracts and returns its PO number so it can be used in subsequent steps.
 */
...
// /**
//  * Finds an approved payment, records it as paid with details, and submits.
//  */
// Cypress.Commands.add('recordNewPayment', (poNumber, paymentDetails) => {
//   cy.log(`--- Recording Payment for PO: ${poNumber} ---`);
//   cy.findPaymentRow(poNumber).within(() => {
//     cy.contains('button', 'Pay').click();
//   });

//   // Assuming a form/modal appears to record payment details (based on test plan FORM-021)
//   // These selectors are educated guesses and should be updated with data-cy if possible.
//   cy.get('body').within(() => {
...
//     cy.get('input[type="file"]').selectFile(paymentDetails.proofFile, { force: true });
...
/**
 * Finds a SPECIFIC approved payment by its PO number and records it as paid.
 * This command still needs the PO number to ensure it's acting on the correct item.
 */
Cypress.Commands.add('recordNewPayment', (poNumber, paymentDetails) => {
