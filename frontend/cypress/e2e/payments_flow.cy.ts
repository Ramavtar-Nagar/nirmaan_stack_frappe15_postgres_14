...
describe('Dynamic Project Payment Approval and Processing Flow', () => {
  ...
  it('should dynamically reject one payment', function () {
    ...
    it('Navigates to New Payments and Verify the newly approved payment presence in the payments table and Record the Payment as Paid', function() {
        ...
        .parent('tr') 
        .within(() => { 
            cy.get('[data-cy="payments-pay-button"]')
                .scrollIntoView()
                .should('be.visible')
                .click({ force: true });
        });
    ...
    it('Veryfies the payment which is paid in the Payments Done tab', function() {
        ...
        cy.get('[data-cy="data-table"]')
            .contains('td', approvedPoNumber) // 1. Find the cell (td) that contains the PO number text
            .should('be.visible')
            .parent('tr') // 2. Go up to its parent row (tr). This is a very stable way to get the correct row.
            .within(() => { // 3. All subsequent commands are now scoped to just this row.
                // 4. Find the link within this row. Since there's only one, this is safe.
                cy.get('a[href*="/project-payments/"]') 
                .click();
            });
        ...
        cy.get('[data-cy="payment-details-button"]', { timeout: 20000 }).should('exist').and('be.visible').click();


        // ------ Right Now The Api Request Issue is there
    ...
    it('Validates the Search and Filter Functionality', function () {
