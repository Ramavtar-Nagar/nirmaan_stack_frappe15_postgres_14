...
import { BadgeIndianRupee, CalendarIcon, CirclePlus, ListChecks, Pencil, Phone, Undo2 } from "lucide-react"
...
export const ProjectForm = () => {
    ...
    const { phase1, setPhase1Data } = useProjectCreationStore();
    const [areaNames, setAreaNames] = useState(phase1.subdivisionNames || []);
    ...
    const {
        // phase1, setPhase1Data,
        phase2, setPhase2Data,
        phase3, setPhase3Data,
        phase4, setPhase4Data,
    ...
    const currentStepFromStore = useProjectCreationStore(state => state.currentStep);
    ...
    const form = useForm<ProjectFormValues>({
        ...
        defaultValues: {
            ...
            project_state: phase2.project_state || "",
            pin: phase2.pin || "",
            phone: String(phase2.phone ?? ""),
            email: phase2.email || "",

            // Phase 3
            // project_duration: phase3.projectDuration || "",
            project_start_date: phase3.projectStartDate ? 
            ...
            project_end_date: phase3.projectEndDate ? 
                (phase3.projectEndDate instanceof Date ? 
                    phase3.projectEndDate : 
                    (phase3.projectEndDate ? new Date(phase3.projectEndDate) : undefined)) : 
                undefined,
    ...
    useEffect(() => {
        const subscription = form.watch((values) => {
            ...
            setPhase2Data({
                ...
                pin: values.pin ? String(values.pin) : undefined,
                email: values.email,
                phone: values.phone ? String(values.phone) : undefined,
            });

            // Phase 3 - Ensure dates are properly handled
            setPhase3Data({
                projectStartDate: values.project_start_date,
                projectEndDate: values.project_end_date,
            });

            // Phase 4
            setPhase4Data({
                projectLead: values.project_lead,
                projectManager: values.project_manager,
                procurementLead: values.procurement_lead,
                designLead: values.design_lead,
                accountant: values.accountant,
            });

            // Phase 5
            setPhase5Data({
                workPackages: values.project_work_packages?.work_packages || [],
            });
        });

        return () => subscription.unsubscribe();
    }, [form, areaNames, setPhase1Data, setPhase2Data, setPhase3Data, setPhase4Data, setPhase5Data]);

   // Sync section with current step on mount and when store updates
    useEffect(() => {
        if (currentStepFromStore) {
            setCurrentStep(currentStepFromStore);
            setSection(sections[currentStepFromStore - 1]);
        }
    ...
    // Updated UseEffect Logic for proper sync with the store
    useEffect(() => {
        if (pincode.length >= 6 && !pincode_data) {
            const update = {
                project_city: "Not Found",
                project_state: "Not Found"
            };
            form.setValue("project_city", update.project_city);
            form.setValue("project_state", update.project_state);
            setPhase2Data(update);
        }
        else if (pincode_data) {
            const update = {
                project_city: pincode_data.city || "",
                project_state: pincode_data.state || ""
            };
            form.setValue("project_city", update.project_city);
            form.setValue("project_state", update.project_state);
            setPhase2Data(update);
    ...
    // Old Diuration Calculation Logic
    // useEffect(() => {
    ...
    // updated duration calculation effect for being more defensive
    useEffect(() => {
        try {
            if (startDate && endDate) {
                // Ensuring that both are Date Objects
    ...
    // Syncing area names with store whenever it changes
    useEffect(() => {
        setPhase1Data({ subdivisionNames: areaNames });
    }, [areaNames, setPhase1Data]);

    // Initializing areaNames from store when component mounts
    useEffect(() => {
        if (phase1.subdivisionNames && phase1.subdivisionNames.length > 0) {
            setAreaNames(phase1.subdivisionNames);
        }
    }, [phase1.subdivisionNames]);

    useEffect(() => {
        if(phase2.pin){
            setPincode(phase2.pin);
            debouncedFetch(phase2.pin);
        }
    }, [phase2.pin])
    ...
    const goToNextSection = async () => {
        ...
        if (isValid) {
            ...
            if (nextIndex <= sections.length) {
                const nextSec = sections[nextIndex - 1];
                setSection(nextSec);
                setCurrentStep(nextIndex);
                goToStepInStore(nextIndex);
            }
        }
    };
    ...
    return (
        ...
        <div className="flex-1 space-y-4">
            ...
            <Steps current={currentStep - 1} className="py-6 px-10"> {/* Ant Design Steps is 0-indexed */}
                    ...
                    <div className="flex flex-col gap-4">
                        {section === "projectDetails" && (
                            <>
                                ...
                                {/* New and Updated logic for Project Gst Form Field according to the store value */}
                                <FormField
                                    control={form.control}
                                    name="project_gst_number"
                                    render={({ field }) => {
                                        // Determine the current selection based on stored value
                                        const currentValue = field.value?.list;
                                        let selectedOption = "Bengaluru"; // default
                                        
                                        if (currentValue?.length === 2) {
                                            selectedOption = "Both";
                                        } else if (currentValue?.[0]?.location === "Gurugram") {
                                            selectedOption = "Gurugram";
                                        }
                                                    ...
                                                    <Select 
                                                        onValueChange={(selectedLocation) => {
                                                            if (selectedLocation === "Both") {
                                                                ...
                                                                field.onChange({ 
                                                                    list: [{ location: "Bengaluru", gst: "29ABFCS9095N1Z9" }] 
                                                                });
                                                            } else {
                                                                field.onChange({ 
                                                                    list: [{ location: "Gurugram", gst: "06ABFCS9095N1ZH" }] 
                                                                });
                                                            }
                                                        }}
                                ...
                                />
                                 {/* Old area names for sub-divisions logic
                                ...
                                {Array.from({ length: Number(form.getValues().subdivisions) || 0 }).map((_, index) => {
                                    const subdivisionName = phase1.subdivisionNames?.[index]?.name || `Area ${index + 1}`;
                                    
                                    return (
                                        <FormItem key={index} className="lg:flex lg:items-center gap-4">
                                            <FormLabel className="md:basis-2/12">Area {index + 1}:</FormLabel>
                                            <div className="md:basis-2/4">
                                                <Input
                                                    type="text"
                                                    ...
                                                    onChange={(e) => {
                                                        // Updating both local state and store
                                                        const newNames = [...(phase1.subdivisionNames || [])];
                                                        if (!newNames[index]) {
                                                            newNames[index] = { name: e.target.value, status: "Pending" };
                                                        } else {
                                                            newNames[index].name = e.target.value;
                                                        }
                                                        setPhase1Data({ subdivisionNames: newNames });
                                                    }}
                                                />
                                            </div>
                                        </FormItem>
                                        ...
                                        <FormItem className="lg:flex lg:items-center gap-4">
                                            ...
                                            <div className="md:basis-2/4">
                                                <FormControl>
                                                    ...
                                                    {/* New Logic for the city according to the pin code from the state */}
                                                    <Input 
                                                        placeholder={phase2.project_city || "City"}
                                                        disabled={true}
                                                        {...field}
                                                        value={phase2.project_city || ""}
                                                    />
                                                </FormControl>
                                                <FormMessage />
                                            </div>
                                        ...
                                        <FormItem className="lg:flex lg:items-center gap-4">
                                            ...
                                            <div className="md:basis-2/4">
                                                <FormControl>
                                                    ...
                                                    {/* New Logic for the state according to the pin code from the state */}
                                                    <Input 
                                                        placeholder={phase2.project_state || "State"} 
                                                        disabled={true} 
                                                        {...field}
                                                        value={phase2.project_state || ""}
                                                    />
                                                </FormControl>
                                                <FormMessage />
                                            </div>
                                ...
                                <div className="flex items-center justify-end gap-2">
                                    {currentStep > 1 && (
                                        <Button variant="outline" onClick={() => {
                                            ...
                                            const prevSec = sections[prevIndex - 1];
                                            setSection(prevSec);
                                            setCurrentStep(prevIndex);
                                            goToStepInStore(prevIndex);
                                        }}>
                                            Previous
                                        </Button>
                                    )}
